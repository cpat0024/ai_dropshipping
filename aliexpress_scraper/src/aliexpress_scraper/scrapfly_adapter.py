...
async def search_products_sf(
    ...
    async def fetch_page(page: int):
        ...
        for it in items:
            ...
            # Extract rating (star rating) - check multiple possible locations
            rating = 0.0
            num_ratings = 0
            orders = 0
            ...
            if isinstance(evaluation, dict):
                if "starRating" in evaluation:
                    try:
                        rating = float(evaluation.get("starRating", 0.0))
                    except (ValueError, TypeError):
                        pass
                if "totalEvaluation" in evaluation:
                    try:
                        num_ratings = int(evaluation.get("totalEvaluation", 0))
                    except (ValueError, TypeError):
                        pass

            # Check trade info as fallback
            if isinstance(trade_info, dict):
                # Look for rating in trade info
                if rating == 0.0:
                    for key in ["avgRating", "starRating", "averageStarRating"]:
                        if key in trade_info:
                            try:
                                rating = float(trade_info.get(key, 0.0))
                                if rating > 0:
                                    break
                            except (ValueError, TypeError):
                                pass
                ...
                # Look for orders/sold count
                for key in ["recentOrderNum", "soldCount", "tradeCount", "tradeNum"]:
                    if key in trade_info:
                        try:
                            orders = int(trade_info.get(key, 0))
                            if orders > 0:
                                break
                        except (ValueError, TypeError):
                            pass
...
async def scrape_product_and_store_sf(
    ...
    try:
        ...
        # Strategy 1: AGGRESSIVE - Look for ANY span with a decimal number between 0-5
        # This catches ratings like <span data-spm-anchor-id="...">4.8</span>
        ...
        # Strategy 2: Look for rating in specific span elements with data attributes
        if not rating:
            rating_selectors = [
        ...
        # Strategy 3: Extract from structured data in scripts
        if not rating:
            scripts = sel.xpath(
        ...
        # Strategy 4: Fallback to text-based patterns
        if not rating:
            rating_patterns = [
        ...
        review_selectors = [
            ...
            "//*[contains(@class, 'review-count') or contains(@class, 'rating-count')]//text()",
        ...
        # Strategy 3: Fallback to text-based patterns
        if not num_reviews:
            review_patterns = [
        ...
        if store_url:
            ...
            if specific_store_name and len(specific_store_name.strip()) > 2:
                store_name = specific_store_name.strip()
            else:
                # Strategy 2: Look near store links (but filter carefully)
                ...
                for candidate in link_text_candidates:
                    ...
                    if (
                        ...
                        store_name_candidates.append(candidate)

                # Strategy 3: Look in JSON data for store name
                if not store_name_candidates:
                    scripts = sel.xpath("//script//text()").getall()
                    for script in scripts:
                        name_patterns = [
                            r'"storeName":\s*"([^"]+)"',
                ...
                if store_name_candidates:
                    store_name = store_name_candidates[0]
...
async def scrape_seller_sf(
    ...
    try:
        ...
        # Strategy 1: Look for store rating in specific elements
        store_rating_selectors = [
            "//span[contains(@class, 'store-rating') or contains(@class, 'seller-rating')]//text()",
            ...
            for text in rating_texts:
                ...
                if percent_match:
                    try:
                        ...
                        if 80 <= rating_val <= 100:  # Valid seller rating percentage
                ...
                if star_match and not percent_match:  # Don't double-match
                    try:
                        ...
                        if 0 <= rating_val <= 5:  # Valid star rating
                            rating = rating_val
                            break
                    except ValueError:
                        continue
        ...
        if not rating:
            ...
            for script in scripts:
                rating_patterns = [
                    r'"sellerRating":\s*"?(\d+(?:\.\d+)?)"?',
                    r'"storeRating":\s*"?(\d+(?:\.\d+)?)"?',
                    r'"positiveFeedback":\s*"?(\d+(?:\.\d+)?)"?',
                ]
                for pattern in rating_patterns:
                    match = re.search(pattern, script)
                    if match:
                        try:
                            ...
                            if 0 <= rating_val <= 5:
                                rating = rating_val
                            elif 80 <= rating_val <= 100:
                                rating = rating_val / 20
                            if rating:
                                break
                        except ValueError:
                            continue
