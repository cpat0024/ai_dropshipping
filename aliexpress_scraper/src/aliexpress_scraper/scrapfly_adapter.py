...
async def search_products_sf(sf_cfg: ScrapflyConfig, query: str, limit: int = 20) -> List[Dict]:
    ...
    Returns minimal dicts with productId, store info, and product URL.
    """
...
async def scrape_product_sf(sf_cfg: ScrapflyConfig, url: str) -> Optional[Product]:
    ScrapflyClient, ScrapeConfig, Selector = _require_scrapfly()
    client = ScrapflyClient(key=sf_cfg.key)
    headers = {"accept-language": "en-US,en;q=0.9"}
    if sf_cfg.cookie:
    ...
    num_ratings_text = sel.xpath("//a[contains(@class,'reviewer--reviews')]/text()|//span[@id='j-cnt-review']/text()")
    num_ratings_text = num_ratings_text.get()
...
async def scrape_seller_sf(sf_cfg: ScrapflyConfig, seller_name: str, seller_url: str) -> Seller:
    ...
    location = sel.xpath("//span[contains(@class,'store-loc')]/text()|//*[@data-role='store-location']/text()")
...
async def run_with_scrapfly(query: str, *, max_suppliers: int, max_products_per_seller: int, limit: int, country: str, key: str, cookie: Optional[str] = None) -> ScrapeResult:
    """End-to-end scrape orchestrated via Scrapfly backend to align with our public schema."""
    logger = get_logger()
    sf_cfg = ScrapflyConfig(key=key, country=country, cookie=cookie)
    previews = await search_products_sf(sf_cfg, query, limit=limit)

    # Group by store to produce sellers as in our schema
    sellers_map: Dict[str, Tuple[str, List[str]]] = {}
    for p in previews:
        store = p.get("store", {})
        s_url = store.get("storeUrl") or store.get("storeUrlV2") or ""
        if not s_url:
            # skip if no seller url
            continue
        s_name = store.get("storeName") or "Unknown Seller"
        sellers_map.setdefault(s_url, (s_name, []))[1].append(p["url"])  # type: ignore[index]

    result = ScrapeResult(query=query)
    # Take top sellers
    for i, (s_url, (s_name, urls)) in enumerate(sellers_map.items()):
        if i >= max_suppliers:
            break
        seller = await scrape_seller_sf(sf_cfg, s_name, s_url)
        for u in urls[:max_products_per_seller]:
            try:
                prod = await scrape_product_sf(sf_cfg, u)
                if prod:
                    seller.products.append(prod)
                    await random_sleep(0.3, 0.8)
            except Exception as e:
                logger.warning("Product scrape via Scrapfly failed: %s", e)
