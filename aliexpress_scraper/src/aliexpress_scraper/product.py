...
async def scrape_product(page: Page, url: str) -> Optional[Product]:
    ...
    # Best-effort counts
    rating = await _first_number(
        page, ["span.product-reviewer-satisfaction", "span.overview-rating-average"]
    )  # type: ignore[assignment]
    num_ratings = await _first_int(
        page, ["span.product-reviewer-reviews", "span#j-cnt-review"]
    )  # type: ignore[assignment]
    num_orders = await _first_int(
        page, ["span.product-reviewer-sold", "span#j-order-num"]
    )  # type: ignore[assignment]
...
async def scrape_product_details(page: Page, url: str) -> dict:
    ...
    try:
        ...
        # Basic product info
        title = (
            await _first_text(page, ["h1.product-title-text", "h1", ".product-title"])
            or "Unknown Product"
        )
        price = await _first_text(
            page, [".product-price-value", "span#j-sku-price", ".price-current"]
        )
        original_price = await _first_text(
            page, [".product-price-del", ".price-original"]
        )

        # Detailed product specs
        description = await _first_text(
            page, [".product-description", ".detail-desc", ".product-detail"]
        )
        brand = await _first_text(page, [".product-brand", ".brand-name"])
        category = await _first_text(page, [".product-category", ".breadcrumb"])

        # Shipping and logistics
        shipping_info = await _first_text(
            page, [".product-shipping", ".shipping-info", ".logistics-info"]
        )
        delivery_time = await _first_text(page, [".delivery-time", ".shipping-time"])

        # Seller information
        seller_name = await _first_text(
            page, [".seller-name", ".store-name", ".shop-name"]
        )
        ...
        seller_followers = await _first_int(
            page, [".seller-followers", ".store-followers"]
        )

        # Product ratings and reviews
        rating = await _first_number(
            page, [".product-rating", ".overview-rating-average"]
        )
        ...
        try:
            ...
            review_elements = await page.query_selector_all(
                ".review-item, .feedback-item, .comment-item"
            )
            for element in review_elements[:5]:  # Get first 5 reviews
                review_text = await element.query_selector(
                    ".review-content, .comment-content, .feedback-content"
                )
                review_rating = await element.query_selector(
                    ".review-stars, .rating-stars"
                )
                review_author = await element.query_selector(
        ...
        specs = {}
        try:
            spec_elements = await page.query_selector_all(
                ".product-spec, .specification-item, .attr-item"
            )
        ...
        return {
            ...
            "shipping": {"info": shipping_info, "delivery_time": delivery_time},
            "reviews": reviews,
