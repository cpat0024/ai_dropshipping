"""
Fixed version of the scrapfly adapter that handles the new AliExpress data structure.
"""
            ...
            out.append(
                {
                    ...
                    "raw_item": it,  # Keep the full item data for potential future use
                }
            )
        return out
...
async def run_with_scrapfly(query: str, *, max_suppliers: int, max_products_per_seller: int, limit: int, country: str, key: str, cookie: Optional[str] = None) -> ScrapeResult:
    ...
    logger.info(f"Final result: {len(result.suppliers)} suppliers with total products")
    return result
