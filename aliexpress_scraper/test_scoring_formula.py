...
def print_comparison():
    ...
    print("NEW FORMULA: Score = 0.4×(Rating) + 0.3×(log-Sales) + 0.2×(Inventory/Price) + 0.1×(AI-Insight)")
    print("=" * 100)
    print()
    
    for product in test_products:
        print(f"\n📦 {product['name']}")
        print(f"   Rating: {product['rating']}/5 | Orders: {product['num_orders']:,} | Reviews: {product['num_ratings']:,} | Price: ${product['price']}")
        print("-" * 100)
        ...
        print(f"   └─ Rating:     {components['rating_score']}/100 × 0.4 = {components['rating_score'] * 0.4:.1f}")
        print(f"   └─ Sales:      {components['sales_score']}/100 × 0.3 = {components['sales_score'] * 0.3:.1f}")
        print(f"   └─ Inventory:  {components['inventory_score']}/100 × 0.2 = {components['inventory_score'] * 0.2:.1f}")
        print(f"   └─ AI-Insight: {components['ai_insight']}/100 × 0.1 = {components['ai_insight'] * 0.1:.1f}")
